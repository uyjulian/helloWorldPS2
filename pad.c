...
DEFINITION_FOR_EXTERNAL_IRX(mx4sio_bd_irx);
DEFINITION_FOR_EXTERNAL_IRX(mmceman_irx);
DEFINITION_FOR_EXTERNAL_IRX(dev9_irx);
DEFINITION_FOR_EXTERNAL_IRX(extflash_irx);
DEFINITION_FOR_EXTERNAL_IRX(xfromman_irx);
DEFINITION_FOR_EXTERNAL_IRX(poweroff_irx);
...
loadModules(void)
{
    ...
    LOADMODULEBUFFER_EXTERNAL_IRX(mx4sio_bd_irx);
#else
    LOADMODULEBUFFER_EXTERNAL_IRX(mmceman_irx);
#endif
#if 0
    LOADMODULEBUFFER_EXTERNAL_IRX(dev9_irx);
    LOADMODULEBUFFER_EXTERNAL_IRX(extflash_irx);
    LOADMODULEBUFFER_EXTERNAL_IRX(xfromman_irx);
    LOADMODULEBUFFER_EXTERNAL_IRX(poweroff_irx);
#endif
}
...
main()
{
...
#if 0
    {
        iox_stat_t chk_stat;
        char mass_path[] = "mc0:/";
        char mass_path2[] = "mc0:/BIEXEC-SYSTEM";
        for (i = 0; i < 2; i++) {
            mass_path[2] = '0' + i;
            mass_path2[2] = '0' + i;
    ...
    {
        iox_stat_t chk_stat;
        char mass_path[] = "xfrom0:/";
        char mass_path2[] = "xfrom0:/BIEXEC-SYSTEM";
        for (i = 0; i < 1; i++) {
            mass_path[5] = '0' + i;
            mass_path2[5] = '0' + i;
            scr_printf("%s %d\n", mass_path, fileXioGetStat(mass_path, &chk_stat));
...
#if 0
    {
        int fd = fileXioOpen("xfrom0:/BIEXEC-SYSTEM/xosdmain.elf", FIO_O_RDONLY);
        if (fd >= 0)
        {
            int fsize = fileXioLseek(fd, 0, SEEK_END);;
            fileXioLseek(fd, 0, SEEK_SET);

            char *string = malloc(fsize);
            fileXioRead(fd, string, fsize);
            fileXioClose(fd);
...
#if 0
            fd = fileXioOpen("mc0:/BADATA-SYSTEM/pad_example.elf", FIO_O_WRONLY | FIO_O_CREAT);
            if (fd >= 0)
            {
                fileXioWrite(fd, string, fsize);
                fileXioClose(fd);
                scr_printf("Size: %d\n", fsize);
            }
#endif
...
#endif
    {
        int fd = fileXioOpen("mc0:/BIEXEC-SYSTEM/osdmain.elf", FIO_O_RDONLY);
        if (fd >= 0)
        {
            int fsize = fileXioLseek(fd, 0, SEEK_END);;
            fileXioLseek(fd, 0, SEEK_SET);
        ...
        if ((dd = fileXioDopen("mc0:/BADATA-SYSTEM/")) >= 0)
        {
            while (fileXioDread(dd, &record) > 0) {
                scr_printf("X: %s\n", record.name);
                n += 1;
...
#if 0
        // int fd = fileXioOpen("mc0:/BADATA-SYSTEM/", FIO_O_RDONLY);
        // int fd = fileXioOpen("mc0:/BADATA-SYSTEM/pad_example.elf", FIO_O_RDONLY);
        int fd = fileXioOpen("host:/Users/julian/Documents/ps2src/uyjulian/padman_compat_test/pad_example.elf", FIO_O_RDONLY);
        if (fd >= 0)
        {
            int fsize = fileXioLseek(fd, 0, SEEK_END);;
            fileXioLseek(fd, 0, SEEK_SET);
        ...
        fd = fileXioOpen("mc0:/BADATA-SYSTEM/pad_example.elf", FIO_O_RDONLY);
        if (fd >= 0)
        {
            int fsize = fileXioLseek(fd, 0, SEEK_END);;
            fileXioLseek(fd, 0, SEEK_SET);
            ...
            fd = fileXioOpen("host:/Users/julian/Documents/ps2src/uyjulian/padman_compat_test/pad_example_fixme.elf", FIO_O_WRONLY | FIO_O_CREAT);
            if (fd >= 0)
            {
                fileXioWrite(fd, string, fsize);
                fileXioClose(fd);
