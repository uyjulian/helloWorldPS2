...
DEFINITION_FOR_EXTERNAL_IRX(ps2mouse_irx);
...
loadModules(void)
{
    ...
    LOADMODULEBUFFER_EXTERNAL_IRX(ps2mouse_irx);
}
...
main()
{
    ...
    int mouse_init = PS2MouseInit();

    scr_printf("MouseInit: %d\n", mouse_init);
    ...
    for (;;) {      // We are phorever people
        ...
        }
        {
            ...
            if (memcmp(&old_mousedata, &mousedata, sizeof(mousedata)) != 0)
            {
                do_output("MOUSE");
            ...
            memcpy(&old_mousedata, &mousedata, sizeof(mousedata));
        }
