...
WriteMemoryCallback(void *contents, size_t size, size_t nmemb, void *userp)
{
  ...
  printf("Network: Writing %d bytes\n", realsize);
  if(mem->memory == NULL) {
    /* out of memory */
...
int main(int argc, char *argv[])
{
    ...
    curl = curl_easy_init();
    if(curl) {
        curl_easy_setopt(curl, CURLOPT_URL, "https://github.com/BoGanon/ps2sdk/commit/cbbb9bf14e29a6162abc054c009622c0f8f96c38.diff");
        ...
        /* Check for errors */
        if(res != CURLE_OK) {
            scr_printf("curl_easy_perform() failed: %s\n", curl_easy_strerror(res));
        }
